...
bash ./Tools/simulation/gazebo-classic/sitl_multiple_run.sh -m plane -n 6 -w empty >/tmp/codex_health_patch_fw.log 2>&1 &
FW_PID=$!

sleep 22

cd /home/chen/catkin_ws
roslaunch swarm_bringup swarm_multi_uav_6.launch >/tmp/codex_health_patch_ros.log 2>&1 &
ROS_PID=$!
...
rospy.init_node("uav1_health_patch_diag", anonymous=True)
status_texts = []
...
print("SET_SENS_IMU_MODE", set_int("SENS_IMU_MODE", 1), flush=True)
rospy.sleep(1.0)
